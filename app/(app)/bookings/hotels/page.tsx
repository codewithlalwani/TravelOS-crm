...
export default async function HotelBookingsPage({
  ...
  if (!can(perms, "hotels.view")) redirect(defaultPathFor(perms));
  const canCreate = can(perms, "hotels.create");
  // Hotels don't have a dedicated "edit" permission — reuse create, same as the rest of this module.
  ...
  const providerLogos = await providerLogosForNames("hotel", bookings.map((booking) => booking.hotelDetail?.hotelName));

  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-2xl font-semibold text-foreground">Hotel Bookings</h1>
      ...
      <ListSearch basePath="/bookings/hotels" q={q} placeholder="Search reference, hotel, or customer..." />
            ...
            {bookings.map((b) => (
              <tr key={b.id} className="border-b border-border last:border-0 hover:bg-muted/50">
                <td className="px-4 py-3">
                  ...
                  {b.hotelDetail?.confirmationNo && (
                    <p className="text-xs text-muted-foreground">Conf# {b.hotelDetail.confirmationNo}</p>
                  )}
                </td>
                <td className="px-4 py-3 text-foreground">{b.customer?.name}</td>
                <td className="px-4 py-3 text-foreground">
                  {b.hotelDetail ? <span className="flex items-center gap-2"><ProviderLogo name={b.hotelDetail.hotelName} logoUrl={providerLogos[b.hotelDetail.hotelName]?.url} compact /> {b.hotelDetail.hotelName}</span> : "—"}
                </td>
                <td className="px-4 py-3 text-muted-foreground">
                  {b.hotelDetail ? `${b.hotelDetail.checkIn} → ${b.hotelDetail.checkOut}` : "—"}
                </td>
            ...
            {bookings.length === 0 && (
              <tr>
                <td colSpan={canEdit ? 8 : 7} className="px-4 py-10 text-center text-muted-foreground">
                  {q ? `No hotel bookings match "${q}".` : "No hotel bookings yet. Create your first one to get started."}
                </td>
              </tr>
            )}
          </tbody>
        </table>
        <Pagination basePath="/bookings/hotels" q={q} page={currentPage} pageSize={pageSize} total={total} />
      </div>
    </div>
  );
}
